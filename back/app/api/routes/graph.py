...
async def get_workflow_or_404(
    ...
    if wf is None or getattr(wf, "owner_id") != current_user.id:
...
async def get_workflow_node_or_404(
    ...
    db: AsyncSession = Depends(get_session),
    current_user: UserSchema = Depends(get_current_user),
    wf: Workflow = Depends(get_workflow_or_404),
) -> WorkflowNode:
    node = await workflow_nodes.get_node(db, node_id)

    if node is None or getattr(wf, "owner_id") != current_user.id:
...
async def get_workflow(wf: Workflow = Depends(get_workflow_or_404)):
    return wf
...
async def get_workflow_node(
    node: WorkflowNode = Depends(get_workflow_node_or_404),
):
    return node
